...
listen_thread (void *p)
{
    ...
    do
    {
        ...
        while (recvd < len)
        {
            ssize_t r = recv (fd, data, len - recvd, 0);
            if (r == 0)
            {
                /* Shutdown */
                DEBUG ("RPC[%i]: Shutdown\n", fd);
...
rpc_socket_recv (rpc_socket sock, rpc_id id, void **data, size_t *len, uint64_t waitUS)
{
    ...
    if (waitUS)
    {
        ...
        waitUntil.tv_nsec = (now.tv_usec+waitUS)*1000;
    ...
    do
    {
        ...
        if (m == NULL)
        {
            ...
            {
                pthread_cond_wait (&sock->in_cond, &sock->in_lock);
            }
            sock->waiting--;
        }
    } while (ret != ETIMEDOUT && m == NULL);
    if (m)
    {
...
rpc_socket_create (int fd, rpc_callback cb, rpc_server parent)
{
    ...
    sock->request_cb = cb;
    sock->parent = parent;
    lock_init (&sock->in_lock);
    lock_init (&sock->out_lock);
    lock_init (&sock->lock);
    pthread_cond_init (&sock->in_cond, NULL);
    pthread_create (&sock->thread, NULL, listen_thread, sock);
    return sock;
}
...
rpc_socket_die (rpc_socket sock)
{
    ...
    lock_unlock (&sock->in_lock);
    lock_deinit (&sock->in_lock);
    lock_deinit (&sock->out_lock);
    lock_deinit (&sock->lock);
    free (sock);
    return true;
}
...
rpc_socket_send_s (rpc_socket sock, rpc_id id, void *data, size_t len, uint32_t mode)
{
    ...
    if (send (sock->sock, &hdr, sizeof (hdr), MSG_NOSIGNAL) != sizeof (hdr))
    {
        ERROR ("RPC[%i] Header Send Failed: %s\n", sock->sock, strerror (errno));
    }
    while (sent < len)
    {
...
{
    rpc_id id = 0;
    lock_lock (&sock->out_lock);
    id = sock->next_id++;
    rpc_socket_send_s (sock, id, data, len, MODE_REQUEST);
    lock_unlock (&sock->out_lock);
    return id;
}
...
{
    lock_lock (&sock->out_lock);
    rpc_socket_send_s (sock, id, data, len, MODE_RESPONSE);
    lock_unlock (&sock->out_lock);
    return true;
}
