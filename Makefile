...
all: libapteryx.so apteryx apteryxd apteryx-sync apteryx-alfred
...
apteryx-alfred: alfred.c libapteryx.so
	@echo "Building $@"
...
install: all
	...
	@install -D apteryx-alfred $(DESTDIR)/$(PREFIX)/bin/

clean:
	@echo "Cleaning..."
	@rm -f libapteryx.so apteryx apteryxd apteryx-sync apteryx-alfred *.o *.pb-c.c *.pb-c.h
